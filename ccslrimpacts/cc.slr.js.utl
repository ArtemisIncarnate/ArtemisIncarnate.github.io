let ccSlrMap = L.map('drawSlrMap').setView([38, -98], 4) // center of US coords
...
L.control.layers(baseMapsCcSlr).addTo(ccSlrMap)
...
L.geoJSON(mygeoJsonMapFour, mapOptions).addTo(ccSlrMap) // object 2 is the color or inner object; object 1 is the style with the object 2 within console.log(require('util').inspect(, { depth: null }))
// L.geoJSON(stateDemographics, mapOptions).addTo(ccSlrMap)
